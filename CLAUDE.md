...
5. `FileExporter` 导出变更文件：文本文件用 `diff_engine` 预读的内容字符串，二进制文件通过 `vcs.get_file_content_bytes()` 读取原始字节写入
...
- `FileExporter._is_binary_ext()` 复用同一个扩展名集合判断
- 导出时二进制文件调用 `vcs.get_file_content_bytes()` 读取原始字节，以 `"wb"` 模式写入
...
- **SVN cat / 本地文件**：先 UTF-8，失败回退 GBK（中文 Windows Java 项目）
- **Git 路径**：`_unescape_git_path()` 解码 `core.quotepath` 八进制转义（`\347\274\226` → 编）
- **paichu.txt**：同样 UTF-8 → GBK 回退
